...
async def open_callback(candles, symbol, trade_dt):
    print(candles[-1], symbol, trade_dt)


async def main():
    # prepare context structure for easy later use
    context = {'markets': {}, 'candles': {}}

    # loop = asyncio.get_event_loop()
    # print(f'call_later executed at {time.asctime()}')
    # loop.call_later(1, foo)

    # load all the markets and 1m candles into context var
    await init(context)
